...
let PDFJSClass = (function () {
  ...
  // };
    ...
    return this.parsePage(1, 0, 1.5)
  };

    let parseNextPage = function(instance, idx, id, scale) {
      instance.parsePage(idx, id, scale)
    }

    let writeImageSync = (image) => {
      if (image && image.src) {
        // src is a data URI like data:[<mediatype>][;base64],<data>
        let [,type,enc,data] = image.src.split(/:|;|,/)
        let [name,ext] = type.split('/')
        let dir = fs.mkdtempSync(`${os.tmpdir}${path.sep}image-`)
        let filepath = `${dir}${path.sep}${name}.${ext}`
    ...
    cls.prototype.parsePage = function(idx, id, scale) {
        nodeUtil.p2jinfo("start to parse page:" + (id+1));

        this.pdfDocument.getPage(idx).then((promisedPage) => {
          console.log('parsing page ' + idx)
          let pdfPage = promisedPage;
          ...
          function continueOnNextPage() {
              ...
              if (id === (this.pdfDocument.numPages - 1) ) {
                ...
                process.nextTick(() => this.raiseReadyEvent(null));
                  ...
                  process.nextTick(parseNextPage(this, ++idx, ++id, scale));
              }
          }
          ...
          pageParser.parsePage(
            data => {
                ...
                let page = {
                    Images: pageParser.Images.map(i => {
                      if (i.file) {
                        return { file:i.file }
                      } else {
                        return { file: writeImageSync(i) }
                      }
                    })
                };
    ...
    function swizzleDirectionalPunctuation(text) {
       let str = text.replace(/\(/g,'${OPEN}')
       str = str.replace(/\)/g,'(')
       str = str.replace(/\$\{OPEN\}/g,')')
       return str
     }
     ...
     cls.prototype.getRawTextContent = function() {
         ...
         _.each(this.rawTextContents, function(textContent, index) {
             let prevText = null;
             let prevObj = null;
             let RTL = _.reduce(textContent.bidiTexts, function(result, next){ return result || next.dir === 'rtl' }, false)
             _.each(textContent.bidiTexts, function(textObj, idx) {
               if (prevText) {
                 if (Math.abs(textObj.y - prevText.y) <= 9) {
                   if (RTL && (textObj.x > prevText.x)) {
                     prevText.str = textObj.str + prevText.str;
                   } else {
                   ...
                   if (RTL) {
                     retVal += swizzleDirectionalPunctuation(prevText.str) + "\r\n";
                   } else {
                     retVal += prevText.str + "\r\n";
                   }
                   prevText = textObj;
