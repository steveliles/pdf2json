...
(function () {
    ...
    contextPrototype.putImageData = function(image) {
      if (image.data) {
        try {
          let file = writeImageSync(image)
          addImage({file}, this.canvas.Images)//'data:image/png;base64,' + buffer.toString('base64')}, this.canvas.Images)
        } catch (e) {
          console.error(e)
        }
      } else {
        ...
        //         png.data = image.data
        //         let buffer = PNG.sync.write(png, {})
        //         addImage({ src: 'data:image/png;base64,' + buffer.toString('base64')}, this.canvas.Images)
        //       } else {
        //         addImage(image, this.canvas.Images);
        //       }
        //     };
